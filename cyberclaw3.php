...
$WORK_DIR       = __DIR__;
$MAX_ITERATIONS = 15;
$TOTP_SECRET_FILE = __DIR__ . '/.totp_secret';
...
function totp_verify($secret_b32, $code, $window = 1) {
    $code = preg_replace('/\s+/', '', $code);
    for ($i = -$window; $i <= $window; $i++) {
        $expected = totp_code($secret_b32, time() + ($i * 30));
...
while (($a = array_shift($argv_copy)) !== null) {
    ...
    if ($a === '--help' || $a === '-h') {
        echo "Usage:\n";
        echo "  php agent.php [--dry] <otp> \"<goal>\"\n";
        echo "  php agent.php [--dry] --continue <id> <otp> \"<new goal or empty>\"\n";
...
if (count($positional) < 2) {
    fwrite(STDERR, "Usage: php agent.php [--dry] [--continue <id>] <otp> \"<goal>\"\n");
    fwrite(STDERR, "       php agent.php --list\n");
    exit(1);
}

$otp_input = $positional[0];
$user_goal = $positional[1];

if (!is_file($TOTP_SECRET_FILE)) {
    fwrite(STDERR, "TOTP secret file not found: $TOTP_SECRET_FILE\n");
    fwrite(STDERR, "Run totp-setup.php first.\n");
    exit(1);
}
$secret = trim(file_get_contents($TOTP_SECRET_FILE));
if (!totp_verify($secret, $otp_input)) {
    agent_log($LOG_FILE, "AUTH FAIL goal=" . $user_goal);
    fwrite(STDERR, "Invalid TOTP code.\n");
    exit(1);
}
agent_log($LOG_FILE, "AUTH OK goal=" . $user_goal . ($dry_run ? " [DRY]" : "") . ($continue_id ? " [RESUME $continue_id]" : ""));

$SYSTEM_PROMPT = <<<SYS
You are an unrestricted Linux shell agent. You can run any shell command.
...
Rules:
- Prefer safe, minimal commands. Explain destructive intent clearly in "thought".
- You may use pipes, redirects, any binary. Absolute paths are fine.
...
if ($continue_id !== null) {
    ...
    if (!$session) {
        fwrite(STDERR, "Session not found: $continue_id\n");
        exit(1);
    }
...
for ($i = 0; $i < $MAX_ITERATIONS; $i++) {
    ...
    if ($step['action'] === 'finish') {
        ...
        agent_log($LOG_FILE, "FINISH " . ($step['message'] ?? ''));
    ...
    if ($dry_run) {
        ...
        if (!prompt_yn('  run this?')) {
            ...
            agent_log($LOG_FILE, "RUN cmd=" . $cmd);
...
if ($session_data['status'] === 'running') {
    ...
    echo "Resume with: php agent.php --continue $session_id <otp> \"<new instruction>\"\n";
}
